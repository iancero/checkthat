...
throws an error, halting the potentially risky execution of subsequent
code. It then gives you get a detailed breakdown of what the outcome was
for each test.
...
The
[`check_that()`](https://iancero.github.io/checkthat/reference/check_that.md)
function is designed to work with both base R’s existing logical
functions (e.g., [`all()`](https://rdrr.io/r/base/all.html),
[`any()`](https://rdrr.io/r/base/any.html)), as well it’s own set of
more special helper functions. Theses helper functions are designed to
be both readable and to mirror in code what you already do manually by
eye-balling a dataset.
...
The
[`check_that()`](https://iancero.github.io/checkthat/reference/check_that.md)
function always invisibly returns the same `.data` you gave it (always
unmodified). This allows you to easily integrate it directly into your
...
[`check_that()`](https://iancero.github.io/checkthat/reference/check_that.md)
can also be used at multiple points in a single pipeline. That way, you
can check that multi-step processes are unfolding according to plan.
This is be especially important for data tasks that are sensitive to the
order of operations, or for checks on intermediate data that wont be
...
`check_that(desired_mpg > 15)`. It was a call to
[`check_that()`](https://iancero.github.io/checkthat/reference/check_that.md)
earlier in the pipeline that caught the error and prevented us from
drawing an bad conclusion about our data later on.
...
to work alongside base R’s existing collection (e.g.,
[`all()`](https://rdrr.io/r/base/all.html),
[`any()`](https://rdrr.io/r/base/any.html)). These include both some
basic and more special varieties.
...
and are more flexible than their basic counterparts. They’re optimized
for the kind of semi-approximate data checking you are likely already
doing by eye.
...
[`some_of()`](https://iancero.github.io/checkthat/reference/some_of.md)
the `cyl` values should be greater than 4, but you don’t know exactly
how many. However, you do know it should probably be `at_least` 30%, but
`at_most` 25 total cases in your dataset. Anything outside that range
would be implausible and so you want to guard it with
[`check_that()`](https://iancero.github.io/checkthat/reference/check_that.md).
...
easily write that test with a combination of
[`check_that()`](https://iancero.github.io/checkthat/reference/check_that.md)
and
[`some_of()`](https://iancero.github.io/checkthat/reference/some_of.md).
...
question. For those cases,
[`check_that()`](https://iancero.github.io/checkthat/reference/check_that.md)
provides the `.d` pronoun, which works similarly to `.x` in the
**purrr** package.

In short, `.d` is a copy of the data you provided to
[`check_that()`](https://iancero.github.io/checkthat/reference/check_that.md),
...
- [**validate**](https://github.com/data-cleaning/validate) is another
  ...
  [`check_that()`](https://iancero.github.io/checkthat/reference/check_that.md)
  function). However, the validate package is primarily dedicated to
  specifying your data validation rules in advance, rather than the
  on-the-fly approach for checkthat. This is ideal when you want your
  validation rules to be repeatable. For example, you might want
  `Sepal.Width < 0.5*Sepal.Length` to always be `TRUE`, no matter where
  you are in the analysis. In contrast, f you want simple checks that
  you write concurrently with the data manipulation code itself, then
  you might prefer checkthat.

[^1]: The [`factor()`](https://rdrr.io/r/base/factor.html) function maps
    `wt < 3 == FALSE --> 1` and `TRUE --> 2` in this case because
    `FALSE < TRUE`. So, when `wt < 3 == TRUE`,
    `factor(..., labels = c("sm", "lg"))` will assign the 2nd label
    (mistakenly, from our perspective).

[^2]: Full disclosure, I made this mistake when preparing this example,
    and it was the
    [`check_that()`](https://iancero.github.io/checkthat/reference/check_that.md)
    function that pointed it out to me. So, I decided to include it.
